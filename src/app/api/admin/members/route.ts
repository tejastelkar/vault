import { MEMBER_STATUSES, type MemberStatus } from "@/lib/access/types";
...
export async function GET(request: Request) {
  try {
    ...
    const status = rawStatus && MEMBER_STATUSES.includes(rawStatus as MemberStatus)
    ...
    return Response.json(await listMembersAdmin({ status, search, cursor }));
  } catch (error) {
